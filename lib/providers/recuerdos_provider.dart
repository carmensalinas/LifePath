...
class RecuerdosProvider{
    ...
    Future<String> subirImagen(File imagen) async{
      ...
      var uuid = Uuid();
      final url = Uri.parse('https://lifepath-backend.vercel.app/picture/$uuid');
      final mimeType = mime(imagen.path).split('/');

      final sendImg = http.MultipartRequest('POST',url);

      final file = await http.MultipartFile.fromPath('img', imagen.path, contentType: MediaType(mimeType[0],mimeType[1]));
